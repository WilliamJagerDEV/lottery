...
### Para reverter migrações
    ...
    docker compose run --rm lottery mix ecto.rollback

### Para recriar o banco de dados

    docker compose run --rm lottery mix ecto.reset
...
Para rodar os testes localmente execute o comando:
    ...
    docker compose run --rm lottery mix test

E para rodar todos os testes (`format`, `credo` e `test`) use:

    docker compose run --rm lottery mix ci
...
(`ecto.drop`, `ecto.create`, `ecto.migrate`, `seeds` e `test`) execute:
    ...
    docker compose run --rm lottery mix test.reset
